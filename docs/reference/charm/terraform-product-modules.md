...
The Landscape Scalable product module is a Terraform module that deploys the Landscape Server charm and the applications it depends on (HAProxy, PostgreSQL, RabbitMQ, a TLS certificates provider, Debarchive, and Landscape Task Handler) using Juju, and can optionally deploy and integrate PgBouncer. It is based on the (deprecated) `landscape-scalable` charm bundle, and builds on the {ref}`Landscape Server charm Terraform module <reference-charm-terraform-charm-module>`.

See {ref}`how-to-terraform-juju-deployment` for usage.

Source: [`terraform/product/modules/landscape-scalable`](https://github.com/canonical/landscape-server-operator/tree/main/terraform/product/modules/landscape-scalable) in the `landscape-server-operator` repository.
...
| <a name="input_landscape_task_handler"></a> [landscape\_task\_handler](#input\_landscape\_task\_handler) | Configuration for the Landscape Task Handler charm. Set to null to skip deployment. | <pre>object({<br/>    app_name   = optional(string, "landscape-task-handler")<br/>    channel    = optional(string, "latest/stable")<br/>    charm_name = optional(string, "landscape-task-handler")<br/>    config = optional(map(string), {<br/>      task-handler-snap-channel = "latest/stable"<br/>    })<br/>    constraints = optional(string, "arch=amd64")<br/>    resources   = optional(map(string), {})<br/>    revision    = optional(number)<br/>    base        = optional(string, "ubuntu@24.04")<br/>    units       = optional(number, 1)<br/>    machines    = optional(set(string))<br/>  })</pre> | `null` | no |
