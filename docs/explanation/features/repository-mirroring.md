...
- **Pocket:** Pockets are inside the series. They're system-defined sections of packages within a package repository. Pockets are a concept from the [Ubuntu package archive](https://canonical-ubuntu-packaging-guide.readthedocs-hosted.com/en/latest/explanation/archive/#archivepockets). Landscape uses the following pockets:
